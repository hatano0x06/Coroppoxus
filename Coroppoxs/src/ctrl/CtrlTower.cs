...
/// 敵制御
///***************************************************************************
public class CtrlTower
{

    private List< ActorChTower >    actorChList;
    private List< ActorChTower >    activeList;
...
///---------------------------------------------------------------------------
    ...
    public bool Init()
    {
        actorChList = new List< ActorChTower >();
        if( actorChList == null ){
            return false;
        }

        activeList = new List< ActorChTower >();
        if( activeList == null ){
            return false;
        }
...
//				actorChList[i].ActiveDisPlace = Common.VectorUtil.DistanceXZ(actorChList[i].GetBodyPos(), CtrlRandom.getRandom.CtrlPl.GetPos());
    ...
    {
        ActorChTower actorCh = new ActorChTower(TexId);
        actorCh.Init();
        actorCh.Start();
...
///---------------------------------------------------------------------------
    ...
    /// フレーム：移動
    private bool frameMove( ActorChTower actorCh )
    {
