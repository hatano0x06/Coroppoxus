...
/// 敵制御
///***************************************************************************
public class CtrlMonument
{

    private List< ActorChMonument >    actorChList;
    private List< ActorChMonument >    activeList;
...
///---------------------------------------------------------------------------
    ...
    public bool Init()
    {
        actorChList = new List< ActorChMonument >();
        if( actorChList == null ){
            return false;
        }

        activeList = new List< ActorChMonument >();
        if( activeList == null ){
            return false;
        }
        return true;
...
//				activeList[i].ActiveDisPlace = Common.VectorUtil.DistanceXZ(activeList[i].GetBodyPos(), ctrlResMgr.CtrlPl.GetPos());
    ...
    /// 敵の登録
    public void EntryAddMonument( int chResId, float rotY, Vector3 pos )
    {
        ActorChMonument actorCh = new ActorChMonument();
        actorCh.Init();
        actorCh.Start();
    ...
    /// 敵の登録削除
    public void DeleteEntryMonument( int idx )
    {
	...
	public bool inDistance(Vector3 pos)
	{
		for(int i=0; i< actorChList.Count ; i++){	
			float dis = Common.VectorUtil.DistanceXZ(pos , actorChList[i].GetBodyPos());	
			if(dis < actorChList[i].ShadowSize){
				return true;						
			}
		}
		return false;
...
///---------------------------------------------------------------------------
    ...
    /// フレーム：移動
    private bool frameMove( ActorChMonument actorCh )
    {
