...
/// 敵制御
///***************************************************************************
public class CtrlHouse
{

    private List< ActorChHouse >    actorChList;
    private List< ActorChHouse >    activeList;
...
///---------------------------------------------------------------------------
    ...
    public bool Init()
    {
        actorChList = new List< ActorChHouse >();
        if( actorChList == null ){
            return false;
        }

        activeList = new List< ActorChHouse >();
        if( activeList == null ){
            return false;
        }
...
//				activeList[i].ActiveDisPlace = Common.VectorUtil.DistanceXZ(activeList[i].GetBodyPos(), ctrlResMgr.CtrlPl.GetPos());
    ...
    /// 敵の登録
    public void EntryAddHouse( int TexId ,Vector3 pos)
    {
        ActorChHouse actorCh = new ActorChHouse(TexId);
        actorCh.Init();
        actorCh.Start();
    ...
    /// 敵の登録
    public void EntryAddEnemyHouse( int TexId, Vector3 pos )
    {
        ActorChHouse actorCh = new ActorChHouse(TexId);
        actorCh.Init();
        actorCh.Start();
    ...
    /// 敵の登録削除
    public void DeleteEntryHouse( int idx )
    {
...
///---------------------------------------------------------------------------
    ...
    /// フレーム：移動
    private bool frameMove( ActorChHouse actorCh )
    {
