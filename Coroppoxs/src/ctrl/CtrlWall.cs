...
/// 敵制御
///***************************************************************************
public class CtrlWall
{
    private List< ActorChWall >    actorChList;
    private List< ActorChWall >    activeList;
...
///---------------------------------------------------------------------------
    ...
    public bool Init()
    {
        actorChList = new List< ActorChWall >();
        if( actorChList == null ){
            return false;
        }

        activeList = new List< ActorChWall >();
        if( activeList == null ){
            return false;
        }
	...
	public bool DrawIdx(DemoGame.GraphicsDevice graphDev , int idx){
		if(activeList.Count != 0){
			if(activeList[idx].ActiveFlg == true ){
				if(activeList[idx].ActiveDis < 80){
    ...
    /// 敵の登録削除
    public void DeleteEntryWall( int idx )
    {
    ...
    /// 敵の位置
    public Vector3 GetWallPos( int idx )
    {
