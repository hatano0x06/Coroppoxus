...
public class Sprite
{
	...
	public void UpdatePosition()
	{
		...
		if(Direction.Y != 0.0f){
			...
			int phaseX = (int)( Direction.X * ( 2048.0f / FMath.PI ) ) ;
			float cX = SpriteBatch.sinTable[ ( phaseX + 1024 ) & 4095 ] ;
			float sX = SpriteBatch.sinTable[ phaseX & 4095 ] ;
	
			int phaseY = (int)( Direction.Y * ( 2048.0f / FMath.PI ) ) ;
			float cY = SpriteBatch.sinTable[ ( phaseY + 1024 ) & 4095 ] ;
			float sY = SpriteBatch.sinTable[ phaseY & 4095 ] ;
	
			float ux = cX * Size.X ;	float vx = -sX * Size.Y ;
			float uy =  Size.X ;	float vy =  Size.Y ;
			float uz = sX * Size.X ;	float vz = cX * Size.Y ;
				
			vertexData[ vertexID + 3 ].Position.X = x - ux*Center.X ;
			vertexData[ vertexID + 3 ].Position.Y = y;
			vertexData[ vertexID + 3 ].Position.Z = z - uz*Center.X ;
				
			vertexData[ vertexID + 2 ].Position.X = x - ux*Center.X - sX*sY*Size.Y;
			vertexData[ vertexID + 2 ].Position.Y = y + Size.Y * cY ;
			vertexData[ vertexID + 2 ].Position.Z = z - uz*Center.X - cX*sY*Size.Y;
			
			vertexData[ vertexID + 1 ].Position.X = x + ux*Center.X ;
			vertexData[ vertexID + 1 ].Position.Y = y;
			vertexData[ vertexID + 1 ].Position.Z = z + uz*Center.X ;
			
			vertexData[ vertexID + 0 ].Position.X = x + ux*Center.X - sX*sY*Size.Y ;
			vertexData[ vertexID + 0 ].Position.Y = y + Size.Y * cY ;
			vertexData[ vertexID + 0 ].Position.Z = z + uz*Center.X - cX*sY*Size.Y ;
		}else{							
			int phaseX = (int)( Direction.X * ( 2048.0f / FMath.PI ) ) ;
			float cX = SpriteBatch.sinTable[ ( phaseX + 1024 ) & 4095 ] ;
			float sX = SpriteBatch.sinTable[ phaseX & 4095 ] ;
				
			int phaseZ = (int)( Direction.Z * ( 2048.0f / FMath.PI ) ) ;
			float cZ = SpriteBatch.sinTable[ ( phaseZ + 1024 ) & 4095 ] ;
			float sZ = SpriteBatch.sinTable[ phaseZ & 4095 ];
			...
			/*
			float ux = cZ * Size.X ;	float vx = -sZ * Size.Y ;
			float uy = sZ * Size.X ;	float vy = cZ * Size.Y ;
			...
			*/
			...
			vertexData[ vertexID + 2 ].Position.X = x - ux*Center.X;
			vertexData[ vertexID + 2 ].Position.Y = y + Size.Y ;
			vertexData[ vertexID + 2 ].Position.Z = z - uz*Center.X;
			
			vertexData[ vertexID + 1 ].Position.X = x + ux*Center.X ;
			vertexData[ vertexID + 1 ].Position.Y = y;
			vertexData[ vertexID + 1 ].Position.Z = z + uz*Center.X ;
			
			vertexData[ vertexID + 0 ].Position.X = x + ux*Center.X;
			vertexData[ vertexID + 0 ].Position.Y = y + Size.Y;
			vertexData[ vertexID + 0 ].Position.Z = z + uz*Center.X;
		}
