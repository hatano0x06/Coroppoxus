...
///---------------------------------------------------------------------------
    ...
    public bool FrameLookSelf()
    {
        ...
        isPos.X    = (float)( m_stPos.X + m_nxPos.X );
        isPos.Y    = (float)( m_stPos.Y + m_nxPos.Y );
        isPos.Z    = (float)( m_stPos.Z + m_nxPos.Z );

        objCam.SetLookAt( isRot, isPos, isDis ,moveDisX ,selflookRate);

        objCam.FrameLookSelf();
...
///---------------------------------------------------------------------------
    ...
    /// カメラパラメータのセット
    public void SetMoveParam( Matrix4 trgMtx, Vector3 movePos, Vector3 moveRot, float moveDis, int moveSpd  ,float moveDisX = 0 , float lookselfRate = 1.0f)
    {
        /// 移動カメラ情報
        ///--------------------------------------------
...
//        m_stDis   = moveDis;
		this.moveDisX = moveDisX;
		this.selflookRate = lookselfRate;
