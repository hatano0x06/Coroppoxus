...
/// ACTOR : モンスターの操作
///***************************************************************************
public class ActorChHobit : ActorChBase
{
...
*/	
	...
	public bool DrawText(DemoGame.GraphicsDevice graphDev){
		if(objSpeech.deadFlag == false){
			objSpeech.DrawText( graphDev ,BasePos );
		}
		return true;
	}
...
///---------------------------------------------------------------------------
	...
	public void setAppearCountSp(int count){
		...
		objSpeech.preDeadflag = false;
		objSpeech.timeCount = 0;
	}

	public void setDeadSp(){
		objSpeech.preDeadflag = true;
	}
...
///---------------------------------------------------------------------------
	...
	{
		objCh.changeId(Id);			
        objSpeech = new ObjSpeechballoon(Id ,0);
        objSpeech.Init();
	}
