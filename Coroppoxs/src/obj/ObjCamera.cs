...
public class ObjCamera : GameObjProduct
{
	...
	private float	   trgDisX;
	private float		lookselfRate;
...
///---------------------------------------------------------------------------
	...
	public bool FrameLookSelf(){
        camCore.SetPerspective( dspWidth, dspHeight, camAngle, camNear, camFar );
        camCore.SetLookSelf( trgRot, trgPos, trgDis ,trgDisX ,lookselfRate);
        return true;
	}
...
///---------------------------------------------------------------------------
    ...
    /// Look Atのパラメータセット
    public void SetLookAt( Vector3 trgRot, Vector3 trgPos, float trgDis ,float trgDisX = 0 ,float lookselfRate = 1.0f)
    {
        this.trgPos = trgPos;
        this.trgRot = trgRot;
        this.trgDis = trgDis;
		this.trgDisX = trgDisX;
		this.lookselfRate = lookselfRate;
